...
function emojiShow(){
    $("#emoji").popover('show')
    if(emojiloadState==0){
        $('.emt_list li').attr('onclick', 'a(this)');
        emojiloadState = 1;
    }
}
...
//输出到输入框
function a(dom){
    var text =dom.getAttribute("_text")
    var img = dom.innerHTML;
    var val = $("#DataToSend").val();
    $("#DataToSend").val(val+text);
}
//解析表情
function analysisEmoji(text){
    var reg  = /\[s[\s\S]*?\]/
    var t
    while(reg.exec(text)!=null){
        t = reg.exec(text)
        text = text.replace(reg,$("li[_text='"+t[0]+"']")[0].innerHTML);
    }
    return text;
}
...
$(document).ready(function () {
   ...
   $("#emoji").attr('data-template',emojiData)
